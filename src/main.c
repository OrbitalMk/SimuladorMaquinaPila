...
on_activate (GtkApplication *app)
{
  ...
  gtk_css_provider_load_from_resource (provider, "/org/gnome/simulador/resource/style.css");
  gtk_style_context_add_provider_for_screen (gdk_screen_get_default (),
	...
	if (window == NULL)
		window = g_object_new (SIMULADOR_TYPE_WINDOW,
		                       "application", app,
		                       "default-width", 800,
		                       "default-height", 600,
		                       NULL);
      ...
      char *argv[])
{
	 ...
	 */
	app = gtk_application_new ("org.gnome.simulador", G_APPLICATION_FLAGS_NONE);
